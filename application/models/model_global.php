...
class Model_global extends CI_Model {
    ...
    public function view_trans($limit,$start,$where){ //menampilkan transaksi pagination
    ...
    public function insert($data, $table){ //memasukan data
    ...
    public function update($id, $data, $table, $where){ //perbaharui data
        $this->db->set($data);
		$this->db->where($id,$where);
		$this->db->update($table); 
    }

    public function delete($table, $where){ //hapus data
    ...
    public function update_total($id,$data,$data2,$field,$table,$where){ //update total bayar
        $this->db->set($field,$data+$data2,FALSE);
        $this->db->where($id,$where);
        $this->db->update($table);
    }

    public function select($id, $field, $table, $where){ //query menampilkan dengan syarat
    ...
    public function view_by($id){ //menampilkan id member
        $this->db->where('id',$id);
        $query=$this->db->get('member');
        return $query->row();
    }

    public function max_id($field_id,$table){ //mendapatkan id terbesar
    ...
    public function sum($field,$id, $table, $where){ // menjumlahkan transaksi detail
    ...
    public function query($query){ //query insert update delete
    ...
    public function upload($field,$file,$id,$where,$table){ //upload bukti pembayaran
